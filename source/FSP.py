...
def FSP_threestate(parameters, N):
    ...
    A = np.array([[-v12-v13, v21, v31], [v12, -v21-v23, v32], [v13, v23, -v31-v32]])
    T = np.diag([k1,k2,k3]); D = np.eye(3)
    AG = sps.lil_matrix((3*N, 3*N), dtype=np.float64)

    for i in range(1,N+1):
        if i<N:
            AG[(i-1)*3:i*3,(i-1)*3:i*3] = AG[(i-1)*3:i*3,(i-1)*3:i*3]+A-T-(i-1)*D
            AG[i*3:(i+1)*3,(i-1)*3:i*3] = T
        else:
            AG[(i-1)*3:i*3,(i-1)*3:i*3] = AG[(i-1)*3:i*3,(i-1)*3:i*3]+A-(i-1)*D
        if i-1>0:
