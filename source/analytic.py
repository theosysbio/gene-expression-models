...
def memoize(f):
    """Creates a dictionary that stores all previous evaluations of a given function"""
    memo = {}
    def helper(x):
        if x not in memo:
            memo[x] = f(x)
...
def analytic_telegraph(parameters, N):
    ...
    for n in range(N):
        ...
        if np.isinf(P[n]):    # Use Stirling's approximation for n!
            P[n] = sp.hyp1f1(a+n,b+n,-c) * (c*np.exp(1)/n)**n / np.sqrt(2*n*pi)
...
def twotwo_n(parameters, n):
    ...
    for rc,r0,r1 in r_combinations:
        p += multinomial_memo(rc,r0,r1) * KB**rc * np.exp(-KB) * \
            k0**r0*fracrise_memo(lamda0, mu0, r0) * hyp_memo(lamda0, mu0, k0, r0) * \
            k1**r1*fracrise_memo(lamda1, mu1, r1) * hyp_memo(lamda1, mu1, k1, r1)
...
def twothree_n(parameters, n):
    ...
    r_combinations = [(i,j,k, n-i-j-k) for i in range(n+1) for j in range(n+1-i) for k in range(n+1-i-j)]
...
def analytic_feedback(parameters, N):
    ...
    p00 = Sb*a*sp.hyp1f1(a+1, b, w0)/ru/(Sb-1) - su*sp.hyp1f1(a, b, w0)/R
