...
def recurrence_step_three_switch(prms, M, precision=dfltPrec):
    ...
    def d(n):
        return (n**Decimal(2) + n*(v12 + v13 + v21 + v23 + v31 + v32) + v12*(v23 + v31 + v32) + v13*(v21 + v23 + v32) + v21*(v31 + v32) + v23*v31)
    ...
    def update(nn, ss):
        ...
        for i in range(3):
            ...
            up[i] = kvec[j]*ss[j]*(Decimal(n)*vvec[j, i] + vvec[j, k]*vvec[k, i] + vvec[j, i]*(vvec[k, i] + vvec[k, j])) \
                + kvec[k]*ss[k]*(Decimal(n)*vvec[k, i] + vvec[j, k]*vvec[k, i] + vvec[j, i]*(vvec[k, i] + vvec[k, j])) \
                + kvec[i]*ss[i]*(Decimal(n**2) + vvec[j, i]*vvec[k, i] + vvec[j, k]*vvec[k, i] + vvec[j, i]*vvec[k, j] \
    ...
    ini = [(vvec[1, 0]*vvec[2, 0] + vvec[1, 2]*vvec[2, 0] + vvec[1, 0]*vvec[2, 1])/d(0),
             (vvec[0, 1]*vvec[2, 0] + vvec[0, 1]*vvec[2, 1] + vvec[0, 2]*vvec[2, 1])/d(0),
            (vvec[0, 2]*vvec[1, 0] + vvec[0, 1]*vvec[1, 2] + vvec[0, 2]*vvec[1, 2])/d(0)]
